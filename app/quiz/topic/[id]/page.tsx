...
export default function TopicQuizPage({ params }: TopicQuizPageProps) {
  ...
  useEffect(() => {
    const initializeQuiz = async () => {
      ...
      const { data: questionsData } = await supabase
        ...
        .limit(5) // Limit to 5 questions per topic quiz
  ...
  const startQuiz = async () => {
    ...
    const { data: attemptData, error } = await supabase
      ...
      .insert({
        user_id: user.id,
        subject_id: topic.chapter.book.subject.id,
        quiz_type: "mixed",
        ...
        <div className="flex items-center justify-center py-20">
          <Card className="max-w-md">
            <CardHeader>
              ...
              <CardDescription>There are no practice questions available for this topic yet.</CardDescription>
            </CardHeader>
            <CardContent>
              <Link href={`/topics/${topicId}`}>
                <Button className="w-full">Back to Topic</Button>
              </Link>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-4">
              ...
              <Badge
                style={{
                  backgroundColor: `${topic.chapter.book.subject.color}20`,
                  color: topic.chapter.book.subject.color,
                }}
              >
        ...
        {!quizStarted && !quizCompleted && (
          <div className="text-center">
            <Card className="max-w-2xl mx-auto">
              <CardHeader>
                <CardTitle className="text-2xl">Practice Quiz: {topic.title}</CardTitle>
                <CardDescription>Test your understanding with {questions.length} practice questions</CardDescription>
              </CardHeader>
        ...
        {quizStarted && !quizCompleted && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <h1 className="text-2xl font-bold text-gray-900">{topic.title} - Practice Quiz</h1>
          ...
          <div className="text-center space-y-6">
            <Card className="max-w-2xl mx-auto">
              <CardHeader>
                ...
                <CardDescription>Here are your results for {topic.title}</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
